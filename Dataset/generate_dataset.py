...
# Add your Gemini API key here before running
API_KEY = ""
...
def build_clinical_description(row):
    """Converts a metadata.csv row into a structured clinical text description
    used as context in the Gemini prompt."""
...
def generate_qa_pair(clinical_description, diagnosis, max_retries=5):
    """Calls Gemini to generate a QA pair for a given clinical case.
...
def generate_dataset(input_csv, output_file):
    ...
    print(f"Total usable rows: {len(usable_rows)}")
    ...
    # Resume from existing output file if interrupted mid-run
    dataset = []
    try:
    ...
    # Entries saved before row_index tracking was added have no index — skip them by count
    fallback_skip = len(dataset) - len(done_indices)
    fallback_counter = 0
    ...
    print(f"Saved to: {output_file} and {txt_file}")


if __name__ == "__main__":
    generate_dataset(
        input_csv="metadata.csv",
        output_file="skin_cancer_dataset_gemini40.json",
    )